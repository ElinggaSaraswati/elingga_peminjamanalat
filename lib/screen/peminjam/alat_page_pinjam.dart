...
class _AlatPageState extends State<AlatPage> {
  ...
  final Stream<List<Map<String, dynamic>>> _alatStream = 
      Supabase.instance.client.from('alat').stream(primaryKey: ['id']);
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              Text("Hi, ${widget.role == 'admin' ? 'Admin' : 'Peminjam'}",
                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              const Text("zalras@gmail.com", style: TextStyle(fontSize: 12, color: Colors.grey)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildAlatCard(Map<String, dynamic> item) {
    bool isTersedia = item['status'] == "Tersedia";
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text("Kat: ${item['kategori']}", style: const TextStyle(fontSize: 10, color: Colors.grey)),
                const SizedBox(height: 5),
                _buildStatusBadge(isTersedia),
              ],
            ),
          ),
          _buildActionButton("Pinjam", const Color(0xff1B607A), Icons.add_shopping_cart),
        ],
      ),
    );
  }

  Widget _buildStatusBadge(bool isTersedia) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
      decoration: BoxDecoration(
        color: isTersedia ? Colors.teal[50] : Colors.red[50],
        borderRadius: BorderRadius.circular(8),
      ),
      child: Text(isTersedia ? "Tersedia" : "Dipinjam", 
        style: TextStyle(color: isTersedia ? Colors.teal : Colors.red, fontSize: 10, fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget _buildFabAdmin() => FloatingActionButton(onPressed: (){}, child: const Icon(Icons.add));
  Widget _buildFabPeminjam() => FloatingActionButton(onPressed: (){}, backgroundColor: Colors.white, child: const Icon(Icons.shopping_cart, color: Colors.black));
}